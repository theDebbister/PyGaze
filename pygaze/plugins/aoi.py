...
class AOI:
    ...
    def __init__(self, aoitype, pos, size):
        ...
        if aoitype not in allowed_aoi:
            raise Exception(
                "Error in libgazecon.AOI.__init__: aoitype {} not recognized; use one of {}".format(
                    aoitype, allowed_aoi
                    )
                )
        ...
        if type(size) in [int, float]:
            ...
            raise Exception(
                "Error in libgazecon.AOI.__init__: size should be either an integer value or a [width,height] list of integer values"
                )
        ...
        if self.aoitype == "circle" and self.size[0] != self.size[1]:
            raise Exception(
                "Error in libgazecon.AOI.__init__: libgazecon.AOI.__init__: a circle does not have different width and height arguments! Either use 'ellipse' as aoitype or a single integer size value"
                )
    ...
    def contains(self, pos):
        ...
        if self.aoitype == "circle":
            ...
            if ((self.pos[0] - pos[0]) / (self.size[0] / 2)) ** 2 + (
                    (self.pos[1] - pos[1]) / (self.size[1] / 2)) ** 2 <= 1:
                return True
            else:
                return False
            ...
            raise Exception(
                "Error in libgazecon.AOI.contains: unknown aoitype {}; use one of 'rectangle', 'circle', 'ellipse'".format(
                    self.aoitype
                    )
                )
