...
class TobiiGlassesController():
    ...
    def __discover_device__(self):
        ...
        s6.bind(('::', PORT))
        s6.sendto('{"type":"discover"}'.encode("utf-8"), (MULTICAST_ADDR, 13006))
        data, address = s6.recvfrom(1024)
        log.debug("From: " + address[0] + " " + data.decode("utf-8"))
    ...
    def wait_until_is_calibrated(self, calibration_id):
        ...
        if status == 'calibrated':
            ...
            log.debug("Calibration {} failed ".format(calibration_id))
            return False
    ...
    def wait_until_recording_is_done(self, recording_id):
        ...
        if status == 'done':
            log.debug("Recording {} successfully stored".format(recording_id))
            return True
        else:
            log.debug("Recording {} failed".format(recording_id))
            return False
    ...
    def create_participant(self, project_id, participant_name="DefaultUser", participant_notes=""):
        ...
        if participant_id is None:
            ...
            log.debug("Participant {} already exists ...".format(participant_id))
            return participant_id
