...
if sys.maxsize > 2 ** 32:
    ...
    print(
        "Python Alea: detected 64-bit application; will use '{}'".format( \
            dll_name
        )
    )
else:
    dll_name = "CEtAPI.dll"
    print(
        "Python Alea: detected 32-bit application; will use '{}'".format( \
            dll_name
        )
    )
...
class AleaTracker:
    ...
    def __init__(self, app_key, file_path="alea_default.csv", target_ip=None, \
        ...
        if self._debug:
            self._debug_log(
                "Alea connection: opening with target IP={} port={}, listen IP={} port={}".format( \
                    target_ip, target_port, listen_ip, listen_port
                )
            )
        self.api.Open(
            app_key, targetIP=target_ip, targetPort=target_port, \
        ...
        print(
            "Successfully connected to Alea API, version={}, device={}".format( \
        ...
        while self._connected.is_set():
            ...
            # Process samples from the Queue.
                ...
                if type(sample) in [tuple, list]:
                    ...
                    print(
                        "WARNING: Unrecognised object in log queue: '{}'".format( \
                            sample
                        )
                    )
    ...
    def _stream_samples(self):
        ...
        while self._connected.is_set():
            ...
            if sample is not None:
                if self._debug:
                    self._debug_log(
                        "WaitForData: sample obtained with timestamp {}".format( \
    ...
    def calibrate(self, n_points=9, location=0, randomise=True, \
        ...
        if n_points not in [1, 5, 9, 16]:
            ...
            self.api.Close()
            raise Exception(
                "User requested {} points, but only 1, 5, 9, or 16 are allowed.".format( \
                    n_points
                )
            )
        ...
        if type(location) == int:
            if location not in [0, 1, 2, 3, 4]:
                ...
                self.api.Close()
                raise Exception(
                    "User requested location {}, but only 0, 1, 2, 3 or 4 are allowed.".format( \
                        location
                    )
                )
            ...
            if location not in ["full", "centre", "center", "bottom", "horizontal", "vertical"]:
                ...
                self.api.Close()
                raise Exception(
                    'User requested location {}, but only "full", "centre", "center", "bottom", "horizontal", "vertical" are allowed.'.format( \
                        location
                    )
                )
            if location == "full":
            ...
            self.api.Close()
            raise Exception(
                'User requested location "{}" (type={}), but only "full", "centre", "center", "bottom", "horizontal", "vertical" are allowed.'.format( \
        ...
        if eye not in [0, 1, 2, 3, 4]:
            ...
            self.api.Close()
            raise Exception(
                "User requested eye %d, but only 0, 1, 2, 3 or 4 are allowed." \
                % (eye)
                )
        ...
        if self._debug:
            self._debug_log(
                "PerformCalibration: n_points={}, location={}, randomise={}, slow={}, audio={}, automatic={}, bgc={}, fgc={}, image={}".format( \
                    n_points, location, randomise, slow, audio, automatic, bgc, fgc, image
                )
            )
        self.api.PerformCalibration(
            noPoints=n_points, location=location, \
        ...
        if self._debug:
            self._debug_log(
                "WaitForCalibrationResult: status={}, improve={}".format( \
                    status, improve
                )
            )
...
class AleaAPI:
    ...
    def Open(self, appKey, targetIP=None, targetPort=None, listenIP=None, \
        ...
        if (targetIP is None) or (targetPort is None) or (listenIP is None) or (listenPort is None):
            ...
            r = etapi.Open(
                ctypes.c_char_p(appKey.encode("utf-8")), \
    ...
    def PerformCalibration(self, noPoints=9, location=0, \
        ...
        # Make a call to the API, and save the result in a variable.
        r = etapi.PerformCalibration(
            ctypes.c_int32(noPoints), \
    ...
    def WaitForCalibrationResult(self):
        ...
        # Wait for it.
        r = etapi.WaitForCalibrationResult(
            ctypes.byref(status), \
    ...
    def ShowStatusWindow(self, posX, posY, size, opacity):
        ...
        # Make a call to the API, and save the result in a variable.
        r = etapi.ShowStatusWindow(
            ctypes.c_int32(posX), ctypes.c_int32(posY), \
